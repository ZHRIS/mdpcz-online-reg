 ...
 */
function createAdaptor(serviceLocator, options) {

    var dbProperties = serviceLocator.properties.database
        , logger = serviceLocator.logger
        , serverData
        , db
        , dbOptions = _.extend({ fsync: true, w: 1, 'native_parser': true }, options)
    ...
    db = new Db(dbProperties.name, serverData, dbOptions)
